...
class account(Base):
    __tablename__ = "accounts" # 用户账户数据表
    ...
    is_active = Column(Boolean, default=True)  # 用户的激活状态，默认为激活
...
class questions(Base):
    ...
    question_type = Column(String, ForeignKey("type.question_type")) # 题目类型
...
class mission(Base):
    ...
    uid = Column(Integer, ForeignKey("accounts.uid")) # 用户索引id（外键）
...
class type(Base):
    __tablename__ = "type"
    tyid = Column(Integer, primary_key=True, index=True)  # 添加一个主键
    question_type = Column(String, unique=True, index=True)  # 确保题目类型唯一且有索引
